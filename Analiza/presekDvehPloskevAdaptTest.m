function [K, stKorakov, minMax] = presekDvehPloskevAdaptTest(f1, C1, f2, C2, gradf1, gradf2, X0, korak, funkcija,pf1,pf2,tspan)
  ...
  % izracuna tocke na krivulji preseka in jih vrne v vektorju K, ter min in max korak
  ...
  if(funkcija == 1)
    f = @(t,Y) [F(Y)];
    [t, Y, stKorakov, minMax] = euler(f, tspan, X0, korak, F, f1, C1, f2, C2, gradf1, gradf2);
    K=Y;
  % Izracun po RK4
  elseif(funkcija == 0)
    f = @(t,Y) [F(Y)];
    [t, Y, stKorakov, minMax] = rk4(f, tspan, X0, korak, F, f1, C1, f2, C2, gradf1, gradf2);
    K=Y;
  endif
...
function [t, Y, stKorakov, minMax] = euler(f, tspan, Y0, n, F,f1, C1, f2, C2, gradf1, gradf2)
  ...
  while (k <= (n-1))
    ...
    if (stK < 2) %ce je premajhen povecamo
      h = min(h * 2,100);
      if h != 100
    ...
    elseif(stK > 3) %ce je prevelik zmanjsamo
      h = max(h / 2,0.0000001);
      if h != 0.0000001
        k = k - 1;
      else
...
function [t, Y, stKorakov, minMax] = rk4(f, tspan, Y0, n, F,f1, C1, f2, C2, gradf1, gradf2)
